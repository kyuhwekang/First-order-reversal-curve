...
Voltage across the dielectric/sample
...
Voltage must be the voltage corresponding to the dielectric/sample and must be given in volts.

Charge must be given in coulombs.

Voltage and charge must already use the desired sign convention. The script does not automatically reverse either signal.

The data must contain the initial constant-field baseline interval used for drift correction and peak referencing.

The data must contain the complete FORC pulse sequence expected from the waveform parameters specified in the script.

The final FORC pulse must include its decreasing branch after the reversal point. A file ending at the final peak cannot provide the complete final branch.

Large gaps or missing samples should be avoided. The script warns when a time gap exceeds five times the median sampling interval.
...
estimation of the linear polarization drift, and

estimation of the measured baseline electric field used for pulse detection.
...
\left(\partial P/\partial E_b\right)j
}
{\Delta E{\mathrm{rev}}}.
$$
...
Confirm that the Excel workbook follows the required time-voltage-charge format.

Enter the worksheet location and sample geometry in USER PARAMETERS.

Enter the electric-field waveform parameters actually used in the experiment.

Set repoling_full_sweep to match the measured pulse ordering.

Set t_use_start and t_baseline_end so that the retained baseline interval contains only the initial constant-field portion of the measurement.

Run the script and select the measurement workbook.

Inspect the normalized time trace and extracted FORC branches.

If necessary, enable plot_debug to verify peak identification.

Check the console summary for warnings, reversal-field error, sampling density, and the number of finite $p_{\mathrm{rev}}$ and $p_{\mathrm{irr}}$ values.
